...
{
	mysql_select_db("arrival", $con);
	mysql_query("UPDATE `arrival`.`user_info` SET `nickname` = '$name' WHERE `arrival`.`phone` = '$phone'",$con);
	return 1;
}
...
function addfriend($con,$phone1,$phone2)
{
	...
	if(mysql_num_rows($result)>0)
	{
		...
		if($msg['friends']!=NULL)
			...
			$friends=$phone1;
		mysql_query("UPDATE `arrival`.`friend_info` SET friends ='$friends' where phone=$phone2 ",$con);
	...
	if(mysql_num_rows($result)>0)
	{
		...
		if($msg['friends']!=NULL)
			...
			$friends=$phone2;
		mysql_query("UPDATE `arrival`.`friend_info` SET friends ='$friends' where phone=$phone1 ",$con);
...
#echo $json."<br>";
//echo $obj->api;
$con = mysql_connect("localhost","arrival","arrivalzxcvbnm,./");
magine10255/centos6-lnmp-php56if (!$con)
  {
  die('Could not connect: ' . imagine10255/centos6-lnmp-php56mysql_error());
  }
